...
{
	return reinterpret_cast<pipefs_caches*>(new pipefs::Caches{});
}

void pipefs_caches_destroy(struct pipefs_caches* caches)
{
	delete reinterpret_cast<pipefs::Caches*>(caches);
}
...
{
	reinterpret_cast<pipefs::Caches*>(caches)->release(key);
}

void pipefs_caches_cleanup(pipefs_caches* caches, size_t target_size)
{
	reinterpret_cast<pipefs::Caches*>(caches)->cleanup(target_size);
}
